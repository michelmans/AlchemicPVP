		...
		if (sender instanceof Player && sender.hasPermission(command.getPermission())) {
			if (args.length >= 1) {
				if (Bukkit.getPlayer(args[0]) != null) {
					if (Bukkit.getPlayer(args[0]).teleport((Player)sender, TeleportCause.COMMAND)) {
						Bukkit.getPlayer(args[0]).sendMessage(Messenger.formatString(Messages.TP_TELEPORT.value()));
						return true;
					} else {
